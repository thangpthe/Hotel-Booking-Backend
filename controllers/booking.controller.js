...
export const bookRoom = async (req, res) => {
    try {
        ...
        if (nights <= 0) return res.status(400).json({message: "Invalid dates", success: false});
        ...
        return res.status(200).json({
            success: true, 
            message: "Room booked successfully!",
            booking: { id: booking._id, totalPrice: totalPrice }
        });
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
                price_data: {
                    currency: 'usd',
                    product_data: {
                        name: `${booking.hotel?.hotelName || 'Hotel'} - ${booking.room?.roomType || 'Room'}`,
                    },
            ...
            mode: 'payment',
            success_url: `${clientUrl}/my-bookings?payment=success&bookingId=${booking._id}`,
            cancel_url: `${clientUrl}/my-bookings?payment=cancelled`,
        });
...
export const updateBooking = async (req, res) => {
    try {
        ...
        if (booking.isPaid) {
            return res.status(400).json({ success: false, message: "Cannot edit paid booking" });
        }

        if (booking.status === "Cancelled") {
            return res.status(400).json({ success: false, message: "Cannot edit cancelled booking" });
        }
        ...
        const newTotalPrice = booking.room.pricePerNight * nights * persons;
